<template>
  <div class="crud-opts">
    <span class="crud-opts-left">
      <!--左侧插槽-->
      <slot name="left" />
      <el-button class="filter-item" size="mini" type="primary" icon="el-icon-plus" @click="crud.toAdd">
        Add
      </el-button>
      ...
      <el-button :loading="crud.downloadLoading" :disabled="!crud.data.length" class="filter-item" size="mini"
        type="warning" icon="el-icon-download" @click="crud.doExport">Export</el-button>
      <!--右侧-->
      <slot name="right" />
    </span>
...
export default {
  ...
  methods: {
    ...
    toDelete(datas) {
      this.$confirm(`Confirm deletion of the selected ${datas.length} items?`, 'Tips', {
