        ...
        <el-card class="box-card">
          ...
          <div>
            <div style="text-align: center">
              <div class="el-upload">
                <img :src="user.avatarName ? baseApi + '/avatar/' + user.avatarName : Avatar" title="Click to upload avatar" class="avatar" @click="toggleShow">
                <myUpload
                  v-model="show"
                  :headers="headers"
                  :url="updateAvatarApi"
            ...
            <ul class="user-info">
              <li><div style="height: 100%"><svg-icon icon-class="login" /> UserName<div class="user-right">{{ user.username }}</div></div></li>
              <li><svg-icon icon-class="user1" /> NickName <div class="user-right">{{ user.nickName }}</div></li>
              <li><svg-icon icon-class="dept" /> Department <div class="user-right"> {{ user.dept.name }}</div></li>
              <li><svg-icon icon-class="phone" /> PhoneNumber <div class="user-right">{{ user.phone }}</div></li>
        ...
        <el-card class="box-card">
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="User profile" name="first">
              <el-form ref="form" :model="form" :rules="rules" style="margin-top: 10px;" size="small" label-width="120px">
                ...
                <el-form-item label="">
                  <el-button :loading="saveLoading" size="mini" type="primary" @click="doSubmit">Save configuration</el-button>
                </el-form-item>
              </el-form>
            </el-tab-pane>
            <!--    操作日志    -->
